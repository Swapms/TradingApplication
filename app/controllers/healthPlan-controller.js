  ...
  if (rows.length > 0) {
    ...
    if (rowsmain.length > 0) {
      ...
      jsondata[0].forEach(function (healthplan) {
        ...
        // console.log('start prt : '+ startPtr +'end ptr :'+endPtr)
        ...
        startPtr = startPtr + healthplan.count;
        testTypes = [];
      });
      res.status(200).json({
        status: true,
      ...
      console.log(JSON.parse(JSON.stringify({ Recommended: result, recommendedcount: recommendedcount })))
    }
  }
};
...
exports.getHealthPlanbyId = async (req, res) => {
  ...
  if (rows.length > 0 && plan != null) {
    ...
    if (test_details != '' && test_details != null && test_details != undefined)  
       finalResult = finalResult + plan[test_details] ;
  
    if (other_info != '' && other_info != null && other_info != undefined)  
      finalResult = finalResult + plan[other_info];
